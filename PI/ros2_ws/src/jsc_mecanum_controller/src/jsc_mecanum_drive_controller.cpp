...
controller_interface::return_type MecanumbotDriveController::update(const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    const double min_positive_velocity = 28; 
    const double max_positive_velocity = 200.0;  
    const double min_negative_velocity = -28; 
    const double max_negative_velocity = -200.0;
    ...
    // Array to store last adjusted velocities for each wheel
    static double last_adjusted_velocity[4] = {0.0, 0.0, 0.0, 0.0};

    // Apply limits function with wheel index
    auto apply_limits = [&](double velocity, int wheel_index) -> double {
        if (velocity == 0.0) {
            return velocity; // No limits applied if velocity is zero
        }

        double original_velocity = velocity;

        if (velocity > 0) {
            velocity = std::clamp(velocity, min_positive_velocity, max_positive_velocity);
        } else {
            velocity = std::clamp(velocity, max_negative_velocity, min_negative_velocity);
        }

        // Log and update only if adjusted velocity is different from last adjusted value for this wheel
        if (velocity != original_velocity && velocity != last_adjusted_velocity[wheel_index]) {
            RCLCPP_INFO(get_node()->get_logger(), 
                        "Adjusting velocity for wheel %d from %.2f to %.2f due to limits", 
                        wheel_index, original_velocity, velocity);
            last_adjusted_velocity[wheel_index] = velocity; // Update last adjusted value for this wheel
        }

        return velocity;
    };

    // Apply limits to each wheel velocity
    fl_wheel_velocity = apply_limits(fl_wheel_velocity, 0);
    fr_wheel_velocity = apply_limits(fr_wheel_velocity, 1);
    rl_wheel_velocity = apply_limits(rl_wheel_velocity, 2);
    rr_wheel_velocity = apply_limits(rr_wheel_velocity, 3);
